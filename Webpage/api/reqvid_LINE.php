...
define('UPLOAD_DIR', '../uploads/LINE/vdo/');
...
if ($result->num_rows > 0)
{
	...
	while($row = $result->fetch_assoc())
	{
		...
		if ($user_id == $uid_data && $from == 'LINE')	//เลือกเเถวที่ตรงกับ uid ของผู้ส่งข้อมูล เเละเป็นข้อมูลที่มาจาก line จากนั้นจะทำการใส่ path วีดีโอลงไป ณ ช่องใส่ path วีดีโอ
		{
			echo 'Hi'. "<br>";
